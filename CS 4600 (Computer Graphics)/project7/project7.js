...
function SimTimeStep( dt, positions, velocities, springs, stiffness, damping, particleMass, gravity, restitution )
{
	...
	for (i = 0; i < springs.length; i++) {
		...
		var index0 = springs[i].p0;
		var index1 = springs[i].p1;

		var springLength = positions[index1].sub(positions[index0]).len();
		var springDirection = positions[index1].sub(positions[index0]).div(springLength);
		var lengthChangeSpeed = velocities[index1].sub(velocities[index0]).dot(springDirection);

		var springForce = springDirection.mul(stiffness * (springLength - springs[i].rest));
		var dampingForce = springDirection.mul(damping * lengthChangeSpeed);

		forces[index0].inc(springForce.add(dampingForce));
		forces[index1].dec(springForce.add(dampingForce));
	}
