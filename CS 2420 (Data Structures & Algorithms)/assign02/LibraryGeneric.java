...
public class LibraryGeneric<Type> {
	...
	public ArrayList<LibraryBookGeneric<Type>> lookup(Type holder) {
		ArrayList<LibraryBookGeneric<Type>> tempList = new ArrayList<LibraryBookGeneric<Type>>();
	...
	public ArrayList<LibraryBookGeneric<Type>> getOverdueList(int month, int day, int year) {
		ArrayList<LibraryBookGeneric<Type>> overdueCopy = new ArrayList<LibraryBookGeneric<Type>>();
		for (LibraryBookGeneric<Type> book : library) {
			if (book.getDueDate() != null && book.getDueDate().compareTo(new GregorianCalendar(year, month, day)) < 0) {
				overdueCopy.add(book);
			}
		}
		OrderByDueDate comparator = new OrderByDueDate();
		sort(overdueCopy, comparator);
		return overdueCopy;
	}
	...
	protected class OrderByDueDate implements Comparator<LibraryBookGeneric<Type>> {
		public int compare(LibraryBookGeneric<Type> lhs, LibraryBookGeneric<Type> rhs) {
			return lhs.getDueDate().compareTo(rhs.getDueDate()) > 0 ? 1
					: (lhs.getDueDate().compareTo(rhs.getDueDate()) < 0 ? -1 : 0);
		}
	}
	...
	/** Comparator that defines an ordering among library books using the Title. */
	protected class OrderByTitle implements Comparator<LibraryBookGeneric<Type>> {
		 ...
		 */
		public int compare(LibraryBookGeneric<Type> lhs, LibraryBookGeneric<Type> rhs) {
			return lhs.getTitle().compareTo(rhs.getTitle()) > 0 ? 1
					: (lhs.getTitle().compareTo(rhs.getTitle()) < 0 ? -1 : 0);
		}
	}

	/**
	 * Performs a SELECTION SORT on the input ArrayList. 1. Finds the smallest item
	 * in the list. 2. Swaps the smallest item with the first item in the list. 3.
	 * Reconsiders the list be the remaining unsorted portion (second item to Nth
	 * item) and repeats steps 1, 2, and 3.
	 */
